...
public abstract class FSMState
{
    ...
    //Checks whether a target transform is in front of a source based on angle FOV
    protected virtual bool IsInFront(Transform source, Transform target, float FOVAngleFromCenter)
    {
        Vector3 sourceForward = source.forward;
        Vector3 targetDirection = target.position - source.position;
        targetDirection.Normalize();

        float targetAngleFromCenter = Vector3.Angle(sourceForward, targetDirection);

        return (targetAngleFromCenter <= FOVAngleFromCenter);
    }
